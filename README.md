...
```bash
./qcs_1.2.2 --type qc --input "circuit.txt" --output "mapping.txt" --log INFO
```
...
```bash
./qcs_1.2.2 --type tt --algo rw --input "truth_table.txt" --reduction
```
...
Скрипт создаст директорию `/build` в корне проекта, выполнит сборку исполняемых файлов: `qcs_1.2.2` и `tests` -
