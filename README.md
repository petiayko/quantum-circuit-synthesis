...
Автор проекта не несет ответственность за состояние системы после выполнения программ из настоящего репозитория.
...
Входные данные подаются в виде файла. Выходные данные могут быть записаны в стандартный поток вывода или в файл
...
* `-j arg` или `--jobs arg` определяет максимальное число параллельно выполняющихся задач. Опциональный параметр,
  значение по умолчанию `1`. Допустимые значения аргумента: целое число не менее 1. Если значение аргумента превышает
  максимальное число параллельно выполняющихся задач (это число определено устройством или системой), будет установлено
...
Для генерации случайных подстановок длины `n` можно использовать программу `scripts/sub_generator.py`. Скрипт принимает
на вход число `n` - размер подстановки, путь до файла, в который будет записана созданная подстановка.
...
Для сборки исполняемого файла из сходного кода необходимо в корне запустить скрипт `build_windows.bat` или
`build_linux.sh` в зависимости от платформы.
При этом необходимо наличие следующего программного обеспечения:

* CMake (проект разрабатывался с версией `3.31.4`);
* Ninja (проект разрабатывался с версией `1.12.0`);
* GCC (проект разрабатывался с версией `(MinGW-W64 x86_64-ucrt-posix-seh, built by Brecht Sanders, r8) 13.2.0`);
* G++ (проект разрабатывался с версией `(MinGW-W64 x86_64-ucrt-posix-seh, built by Brecht Sanders, r8) 13.2.0`).
